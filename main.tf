...
resource "aws_lb_target_group" "blue" {
  name        = "strapi-tg-blue-v5"
  port        = 1337
  protocol    = "HTTP"
  target_type = "ip"
  vpc_id      = data.aws_vpc.selected.id
  
  # SPEED FIX: Faster health checks
  health_check {
    path                = "/"
    interval            = 15
    timeout             = 3
...
resource "aws_ecs_service" "strapi" {
  ...
  # SPEED FIX: Remove CodeDeploy controller for the first launch
